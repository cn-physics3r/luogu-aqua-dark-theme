...
- 题目列表/题目页登录态组件：`.filter-section .text.lform-size-middle`、`.filter-section .refined-input.search-text`、`.filter-section .filter-tags .tag-button`、与列表难度标签一致的彩色筛选下拉选项、筛选下拉菜单展开态、`.bottom-wrap.float`、`.l-card .luogu` 题库标签、`.l-card .result .count` 结果统计、`.l-card .header-wrap` 表头分隔线、`.columba-content-wrap.main-content > .sidebar-container.layout > .main > .l-card .title`、`.progress-frame`、题目列表 `.row` 分隔线与末行无横线悬浮状态、`.top-bar`、`.sidebar.lside`、`.user-nav.rside`、`.theme-page.theme-frosted`、`.theme-page.theme-frosted::before`、`.columba-content-wrap.main-content`、`.l-card.header-card`、`.l-card.problem`、`.l-card::before`、`.sidebar-container.reverse.layout`、`.lfe-h2`、`.lfe-h3`。
- 评测记录页：`.full-container:has(> section > .card.padding-default .search-input)` 下的筛选卡片、连体搜索框、状态下拉菜单、记录列表卡片、`.row` 分隔线与不改变行背景的悬浮状态、用户/题目/详情文字、使用主题红绿且仅标签局部高亮的空心 Accepted 与 Unaccepted 状态标签、深色顶部用户导航及白色顶部图标和分页按钮。
- 跨页面组件：表单、按钮、表格、标签页、弹窗、Markdown、代码块、评测状态色和滚动条；题目列表的标签选择弹窗包含专属深色覆盖。

这些选择器是在 2026-07-27 通过已登录 Chrome 核实洛谷主页、`/problem/list`、`P1000` 题目页和 `/record/list` 评测记录页后写入的；同时确认了登录态顶部栏、左右侧栏、首页 Highcharts 图表、评论流、题目列表动态标签弹窗、题目卡片的白色 `::before` 覆盖层，以及评测记录页的筛选卡片和记录行结构。题解和个人页仍未把未经页面确认的专属类名写进主题。
...
- 后续若要继续增强，优先在已登录状态下补充题解和个人主页的专属组件细节。
...
- `/record/list`：筛选卡片、状态下拉菜单、记录行、状态标签、详情文字和分页按钮。
- 移动端宽度：页面宽度、卡片圆角和代码块横向滚动。
- 图片资源：赛事图片、头像和轮播图不应被暗色滤镜处理。

题解和个人主页仍属于未完成的专属覆盖范围，新增选择器前应先在浏览器中确认实际 DOM。
