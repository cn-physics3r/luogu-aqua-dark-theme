...
- 首页登录态旧组件：`.wrapper.wrapped.lfe-body.header-layout.tiny`、`.header-layout > .background`、`main.lfe-body.mobile-body`、`.lg-index-content`、`.lg-article`、`.lg-index-benben .lg-article p[style*="color:red"]`、`#lg-slider`、`.am-panel.lg-index-contest`、`.am-panel.lg-index-contest.am-panel-primary`、`.lg-punch`、`.lg-index-calendar.lg-fg-greendark`、`.lg-fg-red`、`.am-comment-hd`、`.am-comment-bd`、`.highcharts-root`；赛事元数据标签统一使用中性深灰，讨论置顶标签保持独立层级，讨论作者名统一使用主文字色。
